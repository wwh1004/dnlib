...
namespace dnlib.DotNet {
	...
	public struct Importer {
		...
		TypeSig ImportAsTypeSig(Type type, Type declaringType, bool? treatAsGenericInst) {
			if (type is null)
				return null;
			...
			case ElementType.Ptr:		return new PtrSig(ImportAsTypeSig(type.GetElementType(), declaringType, null));
			case ElementType.ByRef:		return new ByRefSig(ImportAsTypeSig(type.GetElementType(), declaringType, null));
			case ElementType.SZArray:	return new SZArraySig(ImportAsTypeSig(type.GetElementType(), declaringType, null));
			...
			case ElementType.Array:
				...
				FixSignature = true;
				return new ArraySig(ImportAsTypeSig(type.GetElementType(), declaringType, null), (uint)type.GetArrayRank(), sizes, lowerBounds);
			...
			case ElementType.GenericInst:
				...
				foreach (var ga in typeGenArgs)
					git.GenericArguments.Add(ImportAsTypeSig(ga, declaringType, null));
				return git;
		...
		TypeSig ImportAsTypeSig(Type type, IList<Type> requiredModifiers, IList<Type> optionalModifiers, Type declaringType) {
			...
			if (IsEmpty(requiredModifiers) && IsEmpty(optionalModifiers))
				return ImportAsTypeSig(type, declaringType, null);

			FixSignature = true;	// Order of modifiers is unknown
			var ts = ImportAsTypeSig(type, declaringType, null);
